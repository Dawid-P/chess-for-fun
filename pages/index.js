...
let calcBefore = (matchId)=>{
  ...
  if(chessDataAllGames[matchId].black.username === 'evad90') result = chessDataAllGames[matchId].black.rating
  if(chessDataAllGames[matchId].white.username === 'evad90') result = chessDataAllGames[matchId].white.rating
  return result
}
...
export default function Home() {
  return (
    <div className={styles.container}>
      ...
      <main>
        <h1>Chess master</h1>
        <h2>Pirc won: {pircWon}</h2>
        <h2>Pirc lost: {pircLost}</h2>
        <h2>Pirc draw: {pircDraw}</h2>
        <h2>Rating difference: {ratingChange}</h2>
      </main>
    </div>
  )
}
